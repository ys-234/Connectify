...
const { ALLMESSAGE_API, SENDMESSAGE_API } = messageEndpoints;
...
const SingleChat = ({ fetchAgain, setFetchAgain }) => {
  ...
  const typingHandler = (e) => {
    ...
    if (!typing) {
      setTyping(true);
      socket.emit("typing", { room: selectedChat._id, userName: user.name }); // Emit chat ID and user name
    }
            ...
            >
              {istyping ? (
                <div>
                  ...
                  /> */}
                  <Text>{`${typingUser} is typing...`}</Text>
                </div>
              ) : (
                <></>
              )}
